...
maximize tumor_treatment: 
    (sum {i in R, j in C, k in B} x[k] * b[i, j, k] * a * t[i, j]) 
    - (sum {i in R, j in C, k in B} x[k] * b[i, j, k] * (1 - a) * c[i, j])
    - (sum {i in R, j in C} (s_1[i, j] + s_2[i, j]));
    #- 0.02 *(max {k in B} (sum {i in R, j in C} b[i, j, k])) * 
    #(sum {i in R, j in C} s_1[i, j] + sum {i in R, j in C} s_2[i, j]);

subject to critical_dose {i in R, j in C}: sum {k in B} x[k] * b[i, j, k] * c[i, j] <= (d_c + s_1[i, j])*c[i, j];
subject to tumor_dose {i in R, j in C}: sum {k in B} x[k] * b[i, j, k] * t[i, j] >= (d_t - s_2[i, j])*t[i, j];
