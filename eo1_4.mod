...
maximize tumor_treatment:
    ...
    sum {j in C} b[i, j, k]*(-(0.9/4900)*(p + 40) * (p - 100) * t[i,j] - (p/100)*c[i, j])
    - sum {k in B} x[k] * sum {i in R} 
    sum {j in C} b[i, j, k] * (p/100)*(1 - c[i, j]) * prod {l in L: (i+l) >= 0 and (j+l) >= 0 and (i+l) < card(R) and (j+1) < card(C)} (1 - c[i+l, j+l]);
